...
from src.explainability.schemas import EXPLANATION_COLUMNS, EXPLANATION_VERSION, SUMMARY_COLUMNS
...
class ExplanationGenerationPipeline:
    ...
    def _load_services(self):
        ...
        predictor = ClaimPredictor.recommended(models_dir=self.models_dir)
        xgb_path = self.models_dir / "xgb_model.pkl"
        if not xgb_path.exists():
            raise FileNotFoundError(f"XGBoost model not found for SHAP explanations: {xgb_path}")
        ...
        try:
            ...
            for _, row in df.iterrows():
                ...
                try:
                    ...
                    logger.exception(
                        "claim_explanation_failed",
                        claim_id=claim.get("claim_id"),
                        error=str(exc),
                    )
                    failed_claims.append({"claim_id": str(claim.get("claim_id")), "error": str(exc)})

            long_df = pd.DataFrame(long_rows)
            if long_df.empty:
                long_df = pd.DataFrame(columns=EXPLANATION_COLUMNS)
            else:
                for col in EXPLANATION_COLUMNS:
                    if col not in long_df.columns:
                        long_df[col] = None
                long_df = long_df[EXPLANATION_COLUMNS]

            summary_df = self._build_summary(long_df)

            long_path = self.table_store.write_table("gold_claim_explanations", long_df)
            summary_path = self.table_store.write_table("gold_claim_explanation_summary", summary_df)
            ...
            report_path = self.output_dir / "explanation_report.json"
